...
async def handle_message(bot, event: CQEvent, _):
    ...
    if service_func.only_to_me and not event['to_me']:
        service_func.sv.logger.info(f'Message {event.message_id} ignored by {trigger_name}: not to me.')
        return  # not to me, ignore.

    if not service_func.sv._check_all(event):
        service_func.sv.logger.info(f'Message {event.message_id} ignored by {trigger_name}: permission denied.')
