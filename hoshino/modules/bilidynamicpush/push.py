...
async def subscribe_dynamic(bot, ev):
    ...
    name = all_user_name.get(text, text)
    saveConfig()
    await bot.send(ev, f'订阅 {name}({text}) 成功')
...
async def disubscribe_dynamic(bot, ev):
    ...
    if not ' ' in text:  # 仅当前群组
        ...
        if text in push_uids.keys():
            uname = all_user_name.get(text, text)
            if str(ev.group_id) in push_uids[text]:
        ...
        if subGroup == 'all':
            ...
            return
    saveConfig()
    await bot.send(ev, f'取消订阅 {uname} 成功')
...
async def reload_config(bot, ev):
    ...
    text = str(ev.message).strip()
    result = ''
    for uid, group_ids in push_uids.items():
        if str(ev.group_id) in group_ids:
            name = all_user_name.get(uid)
            if name:
                result += f'- {name} ({uid})\n'
            else:
                result += f'- {uid}'
