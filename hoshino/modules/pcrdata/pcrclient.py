...
class pcrclient:
    def __init__(self):
        ...
        self.app_version = defaultHeaders.get("APP-VER")
    ...
    async def callapi(
        self, apiurl: str, request: dict, crypted: bool = True, noerr: bool = False
    ):
        ...
        try:
            ...
            if (
                "/source_ini/get_maintenance_status?format=json" == apiurl
                and "store_url" in data_headers
            ):
                # TODO 这里的原理是根据下载链接解析版本，下载链接和渠道版本有关
                # 但这里如果用 autopcr 的 header 会直接报错。
                # 可能是请求格式有差别。不过有 required_manifest_ver 就行了
                match = search(r"(?<=gzlj_)(\d+\.\d+\.\d+)", data_headers["store_url"])
                if not match:
                    print("无法解析版本号，请手动修改constants.py的default_ver")
                else:
    ...
    async def get_maintenance_status(self):
        ...
        while True:
            ...
            if self.app_version != defaultHeaders.get("APP-VER"):
                data["APP-VER"] = self.app_version
            return data
